...
public class Main {
    ...
    public static void searchSeq() {
        ...
        while (i <= endOfArray) {
            ...
            while (i + cnt <= endOfArray && answer[i] == answer[i + cnt]) {
                cnt++;
            }

            // Remove sequence if it meets the condition
            if (cnt >= m) {
                cutArray(i, i + cnt - 1);
                i = 1; // Reset index to recheck from the start
            } else {
                i += cnt; // Skip over this sequence
            }
        }
    }
